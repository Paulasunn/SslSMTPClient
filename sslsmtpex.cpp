...
sslsmtpEx::sslsmtpEx(std::string hostnameMX, int port = 0)
{
    if(port > 0){
        Port = port;
    }
...
bool sslsmtpEx::SendAllMIME(string from, string to, string mimeDATA){

    try{
        // FROM MX
        vector<string> mx = getMX(to,0,0);
        // Send email to each hostname or port (if was send error)
        int mailsend = 0;
        for(int i = 0; i < mx.size(); i++){
            if(mailsend == 0){
                std::string mxhost = mx.at(i);
                cout << "EXTERNAL MX Host " <<  mxhost << endl;

                // !!! DELETE BELOW FOR EXTERNAL HOST !!!
                // mxhost = "localhost";
...
bool sslsmtpEx::SendAll(string from, string to, string replyto, string subject, string msg, string msghtml, vector<string> files){
...
bool sslsmtpEx::SendMIME(string from, string to, string mimeDATA)
{
     try{
        SSL_CTX *ctx;
        int server;
...
void sslsmtpEx::ShowCerts(SSL* ssl)
{   X509 *cert;
    char *line;
...
// Check ssl error
void sslsmtpEx::sslError(SSL *ssl, int received){
